...
export default function AchievementUnlockModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative bg-background rounded-3xl shadow-2xl max-w-sm w-full overflow-hidden border border-border/50">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          <div className="absolute top-0 left-2/3 w-2 h-2 bg-primary/60 rounded-full animate-confetti-5" />
        </div>

        {/* Header */}
        <div className="bg-gradient-to-br from-primary via-primary to-primary px-6 pt-8 pb-16 text-center">
          <div className="flex items-center justify-center gap-2 text-background/85 text-sm font-medium mb-2">
        ...
        <div className="px-6 -mt-12 pb-6">
          <div
            className={`bg-background rounded-2xl shadow-lg border border-border/50 p-6 text-center ${
              isAnimating ? "animate-achievement-pop" : ""
            }`}
          >
            {/* Icon */}
            <div className="w-20 h-20 mx-auto mb-4 bg-warning/15 rounded-2xl flex items-center justify-center text-4xl shadow-inner">
              {icon}
            </div>
          ...
          <button
            onClick={handleNext}
            className="w-full mt-6 py-4 rounded-xl bg-primary text-background font-semibold text-lg hover:bg-primary/90 transition-colors shadow-lg"
          >
...
export function AchievementBadge({
  ...
  return (
    ...
    >
      ...
      <span
        className={`font-medium ${earned ? "text-foreground" : "text-muted-foreground"}`}
      >
        {name}
      </span>
