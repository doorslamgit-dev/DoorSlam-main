...
function getCategoryStyle(code: string): { bg: string; text: string } {
  const styles: Record<string, { bg: string; text: string }> = {
    screen_time: { bg: 'bg-blue-100', text: 'text-blue-600' },
    treats: { bg: 'bg-pink-100', text: 'text-pink-600' },
    activities: { bg: 'bg-green-100', text: 'text-green-600' },
    pocket_money: { bg: 'bg-amber-100', text: 'text-amber-600' },
    privileges: { bg: 'bg-purple-100', text: 'text-purple-600' },
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-lg flex items-center justify-center bg-gray-100">
          <AppIcon name="layout-grid" className="w-5 h-5 text-gray-600" />
        </div>
        <div>
          <h2 className="font-semibold text-gray-900">Reward Catalog</h2>
          <p className="text-sm text-gray-500">Toggle rewards on to make them available</p>
        </div>
      </div>
      ...
      {categories.map((category) => {
        ...
        return (
          <div key={category.id} className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
            {/* Category Header */}
