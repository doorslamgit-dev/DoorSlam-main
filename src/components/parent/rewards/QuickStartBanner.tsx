...
export function QuickStartBanner({ onQuickStart, loading = false }: QuickStartBannerProps) {
  return (
    <div className="bg-white rounded-xl p-4 border border-purple-200 flex flex-col sm:flex-row items-center justify-between gap-3">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 bg-purple-100 rounded-lg flex items-center justify-center">
          <AppIcon name="zap" className="w-5 h-5 text-purple-600" />
        </div>
        <div>
      ...
      <button
        ...
        disabled={loading}
        className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors text-sm font-medium disabled:opacity-50 flex items-center gap-2 whitespace-nowrap"
      >
        {loading ? (
          <>
