...
function getProgressBarColor(coverage: number, priorityTier: string): string {
  const threshold =
    priorityTier === "high" ? 80 : priorityTier === "medium" ? 60 : 40;

  if (coverage >= threshold + 15) return "bg-accent-green";
  if (coverage >= threshold) return "bg-accent-green/80";
  if (coverage >= threshold - 15) return "bg-accent-amber";
...
function getPriorityBadge(tier: string): { label: string; className: string } {
  switch (tier) {
    ...
    case "medium":
      return { label: "Med", className: "text-accent-amber bg-accent-amber/15" };
    default:
